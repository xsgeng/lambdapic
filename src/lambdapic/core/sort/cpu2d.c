...
static void calculate_bucket_index(
    ...
    npy_intp nbin = nx * ny;
    memset(bucket_count, 0, sizeof(npy_intp) * nbin);
...
static PyObject* _calculate_bucket_index(PyObject* self, PyObject* args) {
    ...
    double dx, dy, x0, y0;
    int reverse_x;
    if (!PyArg_ParseTuple(args, "OOOnnnddddOOi",
        &x, &y, &is_dead,
    ...
    calculate_bucket_index(
        ...
        npart, nx, ny, dx, dy, x0, y0,
        (npy_intp*) PyArray_DATA(particle_index),
        (npy_intp*) PyArray_DATA(bucket_count), reverse_x
    );
    Py_RETURN_NONE;
}
...
static npy_intp bucket_sort(
    ...
    for (iattr = 0; iattr < nattrs; iattr++) {
        ...
        // exactly (per-bucket mismatch counts are identical for source and
        // target buckets), so no memset is needed.
...
static PyObject* sort_particles_patches_2d(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "OOOOOOnnddnOOOOOOOOOi",
